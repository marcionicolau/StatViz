performTTest <- function(dataset = "", columnNameX = "", columnNameY = "", paired = "FALSE", alternative = "two.sided", alpha = "0.95")
{
  ...
  result = t.test(x=distributionX, y=distributionY, alternative=eval(parse(text = alternative)), paired=eval(parse(text = paired)), var.equal=TRUE, conf.level=eval(parse(text = alpha)))
